...
function dynamic_visibility_for_elementor_load() {
	...
	$today = date("d-m-Y");
	if( $today >= '22-11-2021' && $today <= '30-11-2021' ) {
		add_action( 'admin_notices', 'dynamic_visibility_for_elementor_black_friday_promo_notice' );
		add_action( 'admin_init', 'dynamic_visibility_for_elementor_black_friday_promo_notice_dismissed' );
	} else {
...
function dynamic_visibility_for_elementor_black_friday_promo_notice() { 
	$user_id = get_current_user_id();
    if ( ! get_user_meta( $user_id, 'dve_black_friday_promo_notice_dismissed' ) ) { ?>
		<div class="error notice-error notice dce-generic-notice is-dismissible">
			<div class="img-responsive pull-left" style="float: left; margin-top: 10px; margin-right: 20px;"><img src="<?php echo DVE_URL; ?>/assets/media/dce.png" title="Dynamic.ooo" height="36" width="36"></div>
			<?php if( defined( 'ELEMENTOR_PRO_VERSION' ) ) { ?>
			<p><strong><?php _e( 'Black Friday Deal, 30% off on Dynamic.ooo!', 'dynamic-visibility-for-elementor' ); ?></strong><br />
				<?php printf( __( 'Want "if this, then that" functionality in your Elementor Pro forms? Forms that can calculate? Or display information dynamically from the backend ANYWHERE in your site, including your WooCommerce shop? %3$sGrab our Black Friday Deal and save 30&#37; with promo code %1$sBLACKWEEK21%2$s%4$s.', 'dynamic-visibility-for-elementor' ), '<strong>', '</strong>', '<a href="https://www.dynamic.ooo/promo/black-week-black-friday-2021/?utm_source=wp-plugins&utm_campaign=plugin-uri&utm_medium=wp-dash-promo">', '</a>' ); ?></p>
			<?php } else { ?>
			<p><strong><?php _e( 'Black Friday Deal, 30% off on Dynamic.ooo!', 'dynamic-visibility-for-elementor' ); ?></strong><br />
				<?php printf( __( 'Do you want to to extend Elementor with over 75 features and display information dynamically from the backend ANYWHERE in your site, including your WooCommerce shop? %3$sGrab our Black Friday Deal and save 30&#37; with promo code %1$sBLACKWEEK21%2$s%4$s.', 'dynamic-visibility-for-elementor' ), '<strong>', '</strong>', '<a href="https://www.dynamic.ooo/promo/black-week-black-friday-2021/?utm_source=wp-plugins&utm_campaign=plugin-uri&utm_medium=wp-dash-promo">', '</a>' ); ?></p>
			<?php } ?>
			<a href="?dve_black_friday_promo_notice_dismissed"><button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php _e('Dismiss this promo', 'dynamic-visibility-for-elementor'); ?></span></button></a>
		</div>
	<?php
	}
}
...
function dynamic_visibility_for_elementor_black_friday_promo_notice_dismissed() {
    $user_id = get_current_user_id();
    if ( isset( $_GET['dve_black_friday_promo_notice_dismissed'] ) ) {
		add_user_meta( $user_id, 'dve_black_friday_promo_notice_dismissed', 'true', true );
	}
}
